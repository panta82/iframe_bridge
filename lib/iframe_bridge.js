...
const HANDSHAKE_MESSAGE_NAME = '___handshake___';
...
function IFrameServer(options = DEFAULT_SERVER_OPTIONS, window = window) {
	if (!options.window) {
		throw new Error(`Window parameter is required`);
	}

	let _handshaking = false;
